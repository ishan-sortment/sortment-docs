...
## Connection Methods & Marketing Analytics Tables
**Sortment Internal Reference — Onboarding Playbook**

This document is a definitive reference for every supported method of connecting HubSpot marketing data into a data warehouse, and for every table that is available in each method. It is structured for use during merchant onboarding — specifically to determine which tables Sortment can expect to find when it is granted read access to a merchant's data warehouse schema, and how to build accurate email campaign analytics from those tables.

Merchants may be running any of several data warehouses — including Snowflake, Amazon Redshift, Google BigQuery, AWS Athena, or Databricks. This document covers all common pipeline methods; where a method or SQL syntax is specific to one warehouse platform it is explicitly noted.
...
|---|---|---|---|---|
| Native Data Share *(Snowflake only)* | HubSpot (built-in) | Ops Hub Professional+ | 15 min (Live) / Daily | Shared DB (read-only) |
...
### 3.3 Optional: Fivetran dbt Package (dbt\_hubspot)
...
## 6. Method E & F: Estuary, Windsor, and Others
...
### Sample SQL — Airbyte/Stitch (unified email\_events table)
...
### Sample SQL — Native Data Share (sharded, V2\_DAILY schema — Snowflake only)
...
## 8. Critical Gotchas & Onboarding Checks
...
|---|---|
| Campaign ID mismatch (Native Data Share) | The OBJECTID in OBJECTS\_CAMPAIGNS is different from PROPERTY\_HS\_EMAIL\_CAMPAIGN\_ID in event tables. HubSpot uses a separate internal content/asset ID. Always verify the join key before writing funnel queries. |
| Fivetran child tables must be enabled | EMAIL\_EVENT only populates if the EMAIL\_EVENT\_\* child tables are explicitly selected in the Fivetran connector's Schema tab. A connector set up without them will have an empty EMAIL\_EVENT table and missing email data. |
| Fivetran 25-hour lookback | Fivetran re-fetches EMAIL\_EVENT\_\* data 25 hours behind the current sync to catch HubSpot's delayed event processing. Expect some duplication in raw tables — use created as the dedup key. |
| Conversion is not a native event | HubSpot does not emit a 'converted' email event. Conversion must be self-defined: typically a deal created/updated, a form submission, or a purchase event within a time window after a click, joined on contact ID. |
| Native Data Share requires Ops Hub Pro+ and Snowflake | The HubSpot Native Data Share is only available to merchants on Operations Hub Professional or Enterprise **and** using Snowflake as their data warehouse. All other warehouse platforms require a third-party ETL connector. |
| Stitch requires Super Admin for email\_events | Stitch specifically requires Super Admin permissions in HubSpot to replicate the email\_events table. If the merchant's integration token was set up with a lesser role, email events will be missing. |
| Airbyte unified vs Fivetran sharded events | Airbyte consolidates all email event types into a single email\_events table with a type column. Fivetran splits them into EMAIL\_EVENT\_SENT, EMAIL\_EVENT\_OPEN, etc. This changes the query pattern — know which pipeline you're on before writing SQL. |
| Metadata query syntax varies by warehouse | The discovery query `SHOW TABLES IN SCHEMA hubspot;` works in Snowflake. Use `INFORMATION_SCHEMA.TABLES` for BigQuery and Redshift (`SELECT table_name FROM information_schema.tables WHERE table_schema = 'hubspot'`). Athena uses Glue catalog; Databricks uses `SHOW TABLES IN hubspot`. |

---

## 9. Onboarding Decision Matrix

Use this table during merchant onboarding to identify which connection method applies and what to request access to.

| If the merchant has... | Recommended path for Sortment read access |
|---|---|
| HubSpot Ops Hub Pro+ **and** Snowflake as their data warehouse | Request access to their Native Data Share (V2\_LIVE or V2\_DAILY schema). Zero ETL infrastructure required, freshest data. *(Snowflake-only feature.)* |
| Fivetran already running to their data warehouse | Confirm EMAIL\_EVENT\_\* child tables are enabled. Ask for read access to the `hubspot` schema. If dbt\_hubspot is running, prefer the mart models (`hubspot__email_campaigns` etc.). |
| Airbyte running to their data warehouse | Request read access to their Airbyte destination schema. Key tables: `email_events` (unified), `campaigns`, `marketing_emails`, `contacts`, `deals`. |
| Stitch running to their data warehouse | Request read access. Verify Super Admin was used for HubSpot auth (required for email\_events). Key tables: `email_events`, `campaigns`, `contacts`, `deals`. |
| No ETL / no warehouse connection yet | Recommend Fivetran (most feature-complete for HubSpot marketing analytics) — it supports all major warehouse destinations. If they are on Snowflake with Ops Hub Pro+, the HubSpot Native Data Share is the fastest path with no ETL to manage. Fivetran dbt package gives you the cleanest analytics layer out of the box. |

When Sortment is granted read access to a merchant's data warehouse schema, the first step should always be to list available tables to confirm which tables exist and which pipeline was used — before writing any analytics SQL. Use the syntax appropriate for the merchant's warehouse (see gotchas section above).
...
- **Fivetran dbt\_hubspot Package (GitHub)** — https://github.com/fivetran/dbt_hubspot
